...
/**
 *	\file       htdocs/core/modules/facture/doc/pdf_facture_multidevises.modules.php
 *	\ingroup    facture
 ...
 */
class pdf_facture_multidevises extends ModelePDFFactures
{
    var $db;
    var $name;
    var $description;
	...
	function _tableau_versements(&$pdf, $object, $posy, $outputlangs)
	{
		...
		// Récupère la devise de Dolibarr
		$doli_devise = $conf->global->MAIN_MONNAIE;
		...
		if ($resql)
		{
			...
			while ($i < $num)
			{
				...
				if ($doli_devise != $obj->devise_code) {
					$pdf->MultiCell(20, 3, price($obj->devise_mt_total, 0, $outputlangs), 0, 'L', 0);
				} else {
		...
		$sql = "SELECT p.datep as date, p.fk_paiement as type, p.num_paiement as num, pf.amount as amount, pf.devise_mt_paiement as devise_mt_paiement,";
		$sql.= " pf.devise_code as devise_code, cp.code";
		...
		if ($resql)
		{
			...
			while ($i < $num) {				
				...
				if ($doli_devise != $row->devise_code) {
					$pdf->MultiCell(20, 3, price($sign * $row->devise_mt_paiement, 0, $outputlangs), 0, 'L', 0);
				} else {
	...
	function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs)
	{
		...
		if ($doli_devise != $result->devise_code) {
			...
			$query_sql = 'SELECT f.rowid, f.devise_mt_total, re.fk_facture_source 
				...
				INNER JOIN ' . MAIN_DB_PREFIX . 'facture as f ON re.fk_facture_source = f.rowid
				WHERE re.fk_facture = ' . $object->id;
	...
	function _pagefoot(&$pdf,$object,$outputlangs,$hidefreetext=0)
	{
		...
		$res = pdf_pagefoot($pdf,$outputlangs,'FACTURE_FREE_TEXT',$this->emetteur,$this->marge_basse,$this->marge_gauche,$this->page_hauteur,$object,0,$hidefreetext);
